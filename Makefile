.PHONY: init install format check test requirements build deploy deploy-ocr deploy-bedrock clean help

# 환경 변수 설정
DEFAULT_LAMBDA ?= ocr_lambda
AWS_PROFILE ?= ddobak
...
# Bedrock Lambda 로컬 테스트
test-bedrock:
	@echo "[INFO] Bedrock Lambda 로컬 테스트 실행 중..."
	uv run python lambdas/bedrock_lambda/handler.py
	@echo "[SUCCESS] Bedrock Lambda 로컬 테스트 완료!"
...
# Bedrock Lambda만 배포
deploy-bedrock:
	@echo "[INFO] Bedrock Lambda 배포 중..."
	./scripts/manager.sh deploy bedrock_lambda
...
# 도움말
help:
	@echo "DDOBAK AI Backend 프로젝트 - OCR & LLM 서비스"
	@echo ""
	@echo "초기 설정:"
	...
	@echo "  make test-bedrock  - Bedrock Lambda 로컬 테스트"
	@echo ""
	@echo "의존성 관리:"
	...
	@echo "  make deploy-bedrock          - Bedrock Lambda 배포"
